...
def plot_3d(density_grid, concentration_grid, time_step):
    ...
    ax1.scatter(X.flatten(), Y.flatten(), Z.flatten(), c=density_grid.flatten(), cmap='inferno')
    plt.colorbar(ax1.collections[0], ax=ax1, label='Density')
    
    ax2 = fig.add_subplot(122, projection='3d')
    ax2.set_title(f"Concentration at time {time_step}")
    ax2.scatter(X.flatten(), Y.flatten(), Z.flatten(), c=concentration_grid.flatten(), cmap='inferno')
